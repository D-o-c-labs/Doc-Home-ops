(() => {
  ...
  const applyPalette = (palette, button) => {
    ...
    const nextHref = link.href.replace(/\/(default|dark)\.css$/, `/${palette}.css`);
  ...
  const init = () => {
    ...
    const preferred = getStoredPalette() || (systemPrefersDark() ? DARK : DEFAULT);
    applyPalette(preferred, button);

    if (button) {
      button.addEventListener("click", () => {
        const current = document.documentElement.dataset.palette === DARK ? DARK : DEFAULT;
